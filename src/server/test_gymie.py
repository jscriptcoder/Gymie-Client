...
class TestGymie(unittest.TestCase):
    ...
    def tearDown(self):
        keys = list(gymie.envs.keys())
        for instance_id in keys:
            del gymie.envs[instance_id]

    def make_env(self, env_id):
        gymie.make(self.ws, env_id)
    ...
    def test_message_handle(self):
        gymie.message_handle(self.ws, 'Wrong JSON')
        self.ws.close.assert_called_with((1003, 'Message `Wrong JSON` is invalid'))

        gymie.message_handle(self.ws, '{"prop": "value"}')
        self.ws.close.assert_called_with((1003, "Message keys ['prop'] are missing or invalid"))

        gymie.message_handle(self.ws, '{"method": "make"}')
        self.ws.close.assert_called_with((1003, "Message keys ['method'] are missing or invalid"))

        gymie.message_handle(self.ws, '{"method": "invalid_method", "params": {}}')
        self.ws.close.assert_called_with((1007, 'Method `invalid_method` not found'))

        gymie.message_handle(self.ws, '{"method": "make", "params": {}}')
        self.ws.close.assert_called_with((1007, 'Parameters `{}` are wrong'))

        gymie.message_handle(self.ws, '{"method": "make", "params": {"env_id": "malformed" }}')
        self.ws.close.assert_called_with((1007, 'Environment `malformed` is malformed'))

        gymie.message_handle(self.ws, '{"method": "make", "params": {"env_id": "NotFound-v1" }}')
        self.ws.close.assert_called_with((1007, 'Environment `NotFound-v1` not found'))

        # TODO: test more exceptions

    def test_make(self):
        with self.assertRaises(gymie.EnvironmentMalformed):
            gymie.make(self.ws, 'malformed')
        
        with self.assertRaises(gymie.EnvironmentNotFound):
            gymie.make(self.ws, 'NotFound-v1')

        gymie.make(self.ws, 'CartPole-v1')
    ...
    def test_lookup_env(self):
        ...
        with self.assertRaises(gymie.InstanceNotFound):
            gymie.lookup_env('not_found')

    def test_reset(self):
        instance_id = self.make_env('CartPole-v1')
        gymie.reset(self.ws, instance_id)
    ...
    def test_close(self):
        ...
        gymie.close(self.ws, instance_id)
        with self.assertRaises(gymie.InstanceNotFound):
            gymie.lookup_env(instance_id)
    ...
    def test_step(self):
        ...
        with self.assertRaises(gymie.WrongAction):
            gymie.step(self.ws, instance_id, 'invalid_action')
        
        # valid action
        action = env.action_space.sample()
        gymie.step(self.ws, instance_id, action)
    ...
    def test_action_sample(self):
        ...
        gymie.action_sample(self.ws, instance_id)
